...
| Tenancy | `gitops`, `tenant`, `tailscale` | A tenant is a stack with its own GitHub Environment; its VMs enrol on the tenant's tailnet and the Builder reaches them over the hypervisor's private bridge. First tenant `qnta` is wired but not yet applied — nothing here counts until its VMs exist, a Builder run lands through the hypervisor, and a destroy removes the device from the tenant's console — [tenants](gitops/tenants.md), ADR-0006 |
