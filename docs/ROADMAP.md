...
| Backup phase 3 | `backup`, `pbs`, `3-2-1` | Proxmox Backup Server as packages on a managed VM (`ark`), chunk store on the NAS over NFS, client-side encryption with the key kept off the node, nightly vzdump of every guest, prune / GC / verify jobs. Restore is a workflow (`09 - PBS Restore`) and has been run: a backup booted. A morning ntfy digest reports last night's run and each guest's newest backup age; a critical rule pages when a covered guest has no backup in 26 h. Still open: Backblaze B2 as a second datastore with a sync job — [backups](gitops/backups.md) |
| Ingress phase 4 | `cloudflare`, `tunnel`, `traefik`, `pocket-id`, `oidc` | The lab from anywhere, for people you name. Cloudflare carries traffic only: the zone's records and the tunnel are OpenTofu (`infra/stacks/cloudflare`, workflow `10 - Cloudflare`), the token read from state. `horizon` runs `cloudflared`, Traefik and Pocket ID as one compose with no host ports; Pocket ID is the passkey login, Traefik's OIDC plugin fronts the homepage, Grafana logs in itself. On the tailnet, `*.lab.<zone>` resolves only through split DNS to horizon, with a Let's Encrypt wildcard and HTTP → HTTPS; Proxmox and PBS offer a Pocket ID realm. Everything proven: from LTE, from the tailnet, "Tunnel is down" fired and cleared, no request reaches a service without a login — [ingress](gitops/ingress.md), ADR-0007 |
...
| Observability phase 2D | `observability`, `grafana`, `prometheus`, `loki` | Alloy on every host; Prometheus, Loki, Grafana and the Proxmox exporter on the machine marked `observability.stack`. Dashboards owned by Git Sync and editable in the UI; datasources, alert rules and delivery provisioned from files. Seven alert rules — guest down, agent silent, disk, memory, node storage, backup missing, backup collector stale — the first five fired deliberately and confirmed delivered to a phone; a morning digest; a homepage with live status — [observability](gitops/observability.md) |
