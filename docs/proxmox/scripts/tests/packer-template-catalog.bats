...
@test "resolve-packer-template.sh resolves debian-12 from catalog" {
  run bash "${SCRIPT_DIR}/../../../scripts/resolve-packer-template.sh" debian-12
  [ "$status" -eq 0 ]
  [[ "$output" == *"AUTOLAB_PACKER_TEMPLATE=debian-12"* ]]
  [[ "$output" == *"AUTOLAB_PACKER_TEMPLATE_FILE=infra/packer/templates/debian-12/debian-12.pkr.hcl"* ]]
  [[ "$output" == *"AUTOLAB_PACKER_TEMPLATE_DIR=infra/packer/templates/debian-12"* ]]
  [[ "$output" == *"AUTOLAB_PACKER_TEMPLATE_STATUS=implemented"* ]]
  [[ "$output" == *"AUTOLAB_PACKER_TEMPLATE_VM_ID=9000"* ]]
  [[ "$output" == *"AUTOLAB_PACKER_TEMPLATE_PROXMOX_NAME=autolab-debian-12-template"* ]]
}
